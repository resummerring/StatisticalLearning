...
class TEST_UnivariateBuilder(unittest.TestCase):
    ...
    def test_single(self):
        ...
        # Standardize
        train_,  test_ = UnivariateBuilder(train, test).standardize().build()
        ...
        # Min-Max scale
        train_, test_ = UnivariateBuilder(train, test).min_max_scale().build()
        ...
        # L2 scale
        train_, test_ = UnivariateBuilder(train, test).l2_scale().build()
        ...
        # Log transform
        train_, _ = UnivariateBuilder(train).log_transform().build()
        expected_train = pd.Series(np.log(train))
        self.assertAlmostEqual(np.linalg.norm(train_ - expected_train), 0, places=10)

        # Power transform
        train_, _ = UnivariateBuilder(train).power_transform(lmbda=1).build()
        expected_train = pd.Series([0, 1, 2, 3, 4])
        self.assertAlmostEqual(np.linalg.norm(train_ - expected_train), 0, places=10)

        train_, _ = UnivariateBuilder(train).power_transform(lmbda=2).build()
        expected_train = train.apply(lambda v: 0.5 * (v ** 2 - 1))
        self.assertAlmostEqual(np.linalg.norm(train_ - expected_train), 0, places=10)

        # Binarize
        train_, test_ = UnivariateBuilder(train, test).binarize(threshold=3.5).build()
        ...
        # Quantize
        train_, test_ = UnivariateBuilder(train, test).quantize(bins=np.array([-1, 2.5, 4.5, 9])).build()
